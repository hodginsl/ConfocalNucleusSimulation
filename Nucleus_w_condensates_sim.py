...
foldername = 'updated_image_arrays_2_pop' #folder to save data
...
psf_height = 1.5 # um
D_protein = 30 # um^2/s; diffusion coefficient of mobile proteins
D_condensate_b = 0.015 #um^2/s; diffusion coefficient of "bound" condensates
D_condensate_m = 0.2 #um^2/s; diffusion coefficient of "mobile" condensates
step_time = 4.1E-6 # s; step time of proteins and condensates
B_protein = 250e4 # Hz; Brightness of protein
B_background = 375e4 #Hz; Brightness of background noise
B_condensate = 15*B_protein #Hz; Brightness of condensate
z_condensate = 6.0
z_slice = 59 # this is a list, to be used as a range in a for loop, position of focal plane along the z-axis
t_slices = 10
...
C_in = 50 # nM; set the nuclear concentration 
    ...
    for k in range(t_slices):
        for j in range(image_array.shape[1]): # x
            for i in range(image_array.shape[0]): # y
                ...
                image_array2[i,j,k] += GaussianBeam(cond_pos, beam_pos, psf_width, psf_height, B_condensate)
